...
const ChatPage = () => {
  ...
  const { sendMessage } = useMessage({
    userId: 'host',
    onMessage: (userName, message, time) => {
  ...
  const onSubmit = (data: z.infer<typeof schema>) => {
    ...
    setMessages(prev => [...prev, { userName: 'me', message: data.message, time: new Date().toISOString() }]);
    form.reset();
  };
  ...
  return (
    <div className="flex flex-col h-dvh">
      ...
      <Header title="CONTACT" showBack={true} />
      ...
      <div ref={messageRef} className="flex-1 overflow-y-auto p-4 space-y-4 bg-gray-100 flex-[1]">
        {messages.map(({ userName, message, time }, index) => (
          <div
            key={index.toString()}
            className={`flex ${userName === 'me' ? 'justify-end flex gap-2  ' : 'justify-end flex-row-reverse'}`}>
            {/* TODO: 시간 추가해냐람 */}
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="flex gap-2">
            <FormField
              ...
              name="message"
              render={({ field }) => (
                <Input {...field} className="flex-1 h-[52px]" placeholder="메시지를 입력하세요..." />
              )}
            />
            <button
              type="submit"
